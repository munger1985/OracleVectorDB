    ...
    with gr.Tab("文字智能图片搜索"):
        with gr.Row():
            with gr.Column(scale=8):
                ...
                t1 = gr.Text(label='输入要搜索的内容')
                # ttype = gr.Text(label='search type')
                ttype =  gr.Radio(["action", "breed", "env"],value='action', label="搜索类型--自动选择")
                slider = gr.Slider(minimum= 1,maximum= 15,interactive=True, step=1, value=4, info="搜索结果限额"),

                s_btn = gr.Button("相似图片查询")
                checkBtn = gr.Button("大模型智能检查")

                # limit = gr.Number(2, interactive=True,
                #                   label="the number of similar images to show")
            with gr.Column(scale=8):
                outputs = gr.Gallery(label="结果图片展示")
